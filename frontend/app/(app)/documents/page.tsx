...
import { Plus, FileBadge2, Archive, RotateCcw, Pencil, AlertTriangle } from "lucide-react";
...
function DocCard({ doc, onEdit }: { doc: Document; onEdit: (doc: Document) => void }) {
  ...
  return (
    <div className={clsx(
      ...
      doc.is_expired && "border-red-300/60 dark:border-red-500/30",
      !doc.is_expired && "border-slate-200 dark:border-white/[0.09]",
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-[15px] truncate" style={{ color: "var(--t-primary)" }}>
              {doc.title}
            </p>
            {doc.doc_type && (
              <p className="text-[12px]" style={{ color: "var(--t-faint)" }}>{doc.doc_type}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={() => onEdit(doc)}
            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-white/[0.08] transition-colors"
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-colors"
            style={{ color: "var(--t-faint)" }}
          >
            <Archive size={13} />
          </button>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[13px]">
        {doc.issued_date && (
          <>
            <span style={{ color: "var(--t-faint)" }}>Выдан</span>
            <span style={{ color: "var(--t-muted)" }}>{formatDate(doc.issued_date)}</span>
          </>
        )}
        ...
        {doc.notify_days_before != null && (
          <>
            <span style={{ color: "var(--t-faint)" }}>Напомнить за</span>
            <span style={{ color: "var(--t-muted)" }}>{doc.notify_days_before} дн.</span>
          </>
        )}
      </div>

      {doc.note && (
        <p className="text-[13px] italic" style={{ color: "var(--t-faint)" }}>{doc.note}</p>
      )}
    </div>
  );
}
...
export default function DocumentsPage() {
  useSetPageTitle({ title: "Документы" });
    ...
    <>
      ...
      <main className="flex-1 p-3 md:p-6 max-w-3xl">
        <div className="flex items-center justify-between mb-5">
          <div>
            <p className="text-[13px]" style={{ color: "var(--t-muted)" }}>
              Отслеживайте сроки действия документов
            </p>
          </div>
          <Button
            variant="primary"
        ...
        {!isLoading && active.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-16 text-center">
            ...
            <p className="text-[13px]" style={{ color: "var(--t-faint)" }}>
              Добавьте паспорт, права, визу или любой другой документ с датой окончания
            </p>
          </div>
        )}
        ...
        {expiringSoon.length > 0 && (
          <section className="mb-5">
            <h2 className="text-[12px] font-bold uppercase tracking-wider mb-2" style={{ color: "var(--t-muted)" }}>
              Истекают в ближайшие 60 дней
            </h2>
            <div className="grid sm:grid-cols-2 gap-3">
