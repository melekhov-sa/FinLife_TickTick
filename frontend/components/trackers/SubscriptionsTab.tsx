  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {sub.total_members > 0 && (
          <AvatarStack avatars={sub.members.map((m) => ({ name: m.contact_name }))} max={3} size="sm" className="shrink-0" />
        )}
      </div>
...
export function SubscriptionsTab() {
  ...
  return (
    <>
      {freshSub && <SubscriptionDetailPanel sub={freshSub} onClose={() => setSelectedSub(null)} />}
      {showCreate && <CreateSubscriptionModal onClose={() => setShowCreate(false)} />}
      {addMemberSubId !== null && <AddMemberModal subId={addMemberSubId} onClose={() => setAddMemberSubId(null)} />}
      ...
      <div className="w-full">
        ...
        {isLoading && (
          <div className="space-y-3">
            {[...Array(3)].map((_, i) => <Skeleton key={i} variant="rect" height={128} className="rounded-[14px]" />)}
          </div>
        )}

        {isError && (
          <div className="text-red-400/70 text-sm text-center mt-12">Не удалось загрузить подписки</div>
        )}
        ...
        {visibleData && (
          <div className="space-y-5">
            <div className="grid grid-cols-3 gap-3">
              {[
                { value: visibleData.length, label: "Подписок", valueClass: "", border: "border-slate-200 dark:border-white/[0.07]", bg: "bg-white dark:bg-white/[0.04]" },
                { value: monthlyExpense > 0 ? monthlyExpense.toLocaleString("ru-RU") + " ₽" : "—", label: "Расход в месяц", valueClass: "text-red-600 dark:text-red-400", border: "border-red-500/20", bg: "bg-red-50 dark:bg-red-500/[0.04]" },
                { value: expiringCount, label: "Истекают ≤14д", valueClass: expiringCount > 0 ? "text-amber-600 dark:text-amber-400" : "", border: expiringCount > 0 ? "border-amber-500/20" : "border-slate-200 dark:border-white/[0.07]", bg: expiringCount > 0 ? "bg-amber-50 dark:bg-amber-500/[0.04]" : "bg-white dark:bg-white/[0.04]" },
              ].map((kpi) => (
                <div key={kpi.label} className={clsx("rounded-[14px] border p-4 min-h-[72px] flex items-center justify-center", kpi.border, kpi.bg)}>
                  <Stat label={kpi.label} value={kpi.value} align="center" size="lg" valueClassName={kpi.valueClass} />
                </div>
              ))}
            </div>

            <div className="flex flex-wrap items-center gap-4">
              <Checkbox checked={showArchived} onChange={(e) => setShowArchived(e.target.checked)} label="Архивные" size="sm" />
              {!showArchived && (
              ...
              <div className="flex items-center gap-2 ml-auto">
                <span className="text-[11px] font-semibold uppercase tracking-widest shrink-0" style={{ color: "var(--t-faint)" }}>Сортировка</span>
                <div className="w-52">
                  <Select value={sort} onChange={(v) => setSort(v as SortKind)} options={SORTS.map((s) => ({ value: s.value, label: s.label }))} />
                </div>
              </div>
            </div>
