    ...
    >
      <div className="w-full max-w-md mx-4 bg-[#1a1d23] border border-white/[0.09] rounded-2xl shadow-2xl">
        ...
        <div className="p-6">
            ...
            {OP_TYPES.map((op) => (
              <button
                ...
                className={`flex-1 py-2 text-xs font-medium rounded-xl border transition-colors ${
                  opType === op.value
                    ? op.activeColor
                    : "bg-white/[0.03] border-white/[0.08] text-white/40 hover:text-white/65 hover:bg-white/[0.05]"
                }`}
              >
                {op.label}
              </button>
          ...
          {!opType && (
            <p className="text-xs text-white/25 text-center py-2">Выберите тип операции выше</p>
          )}
        </div>
      </div>
    </div>
