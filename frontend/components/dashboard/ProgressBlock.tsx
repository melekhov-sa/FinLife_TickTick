...
export function ProgressBlock({ level, efficiency, cells }: Props) {
  ...
  return (
    <div className="bg-white/[0.03] rounded-[14px] border border-white/[0.06] p-5 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-end gap-3 mb-2">
            ...
            <div className="pb-0.5">
              <p className="text-[12px] tabular-nums" style={{ color: "var(--t-secondary)" }}>
      ...
      {/* ── Efficiency ────────────────────────────────────── */}
      {efficiency && (
        <div className={level ? "border-t border-white/[0.05] pt-4" : ""}>
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: "var(--t-faint)" }}>
              Эффективность
            </p>
            <span className={clsx("text-[11px] font-semibold", effTier.color)}>{effTier.label}</span>
          </div>
          <div className="flex items-end gap-2 mb-2">
            <span className={clsx("text-[28px] font-bold tabular-nums leading-none", effScoreColor)} style={{ letterSpacing: "-0.04em" }}>
              {effScore}
            </span>
            <span className="text-[13px] pb-0.5 font-medium" style={{ color: "var(--t-muted)" }}>/ 100</span>
          </div>
          <div className="flex gap-1">
            {Array.from({ length: SCORE_BARS }).map((_, i) => (
              <div
                key={i}
                className={clsx("flex-1 h-4 rounded transition-all", i < effFilledBars ? effBarColor : "bg-white/[0.05]")}
      ...
      {/* ── Habit heatmap ─────────────────────────────────── */}
      {cells.length > 0 && (
        <div className={(level || efficiency) ? "border-t border-white/[0.05] pt-4" : ""}>
          <p className="text-[11px] font-semibold uppercase tracking-widest mb-2" style={{ color: "var(--t-faint)" }}>
            Привычки
          </p>
          <div className="flex gap-[3px]">
            {cells.map((cell) => (
              <div
                ...
                className={clsx("flex-1 h-5 rounded transition-all cursor-default", LEVEL_CLASS[cell.level])}
