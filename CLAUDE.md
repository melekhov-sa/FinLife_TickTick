...
- После изменений Python (роутеры, модели, **скрипты/сиды**) — компиляция:
  `py -m py_compile <файл>` (локальный `.venv` битый, но launcher `py` работает).
  Это ловит синтаксис до деплоя — особенно строки с кавычками/«ёлочками».
